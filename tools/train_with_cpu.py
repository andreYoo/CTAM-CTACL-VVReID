...
def main():
    ...
    graph = models.create('graph', config.MODEL.FEATURES, num_tgt,use_dram=args.use_dram)
    ...
    if args.use_dram==True:
        graph = graph.to(device_cpu)
    else:
    ...
    # Trainer
    trainer = Trainer(config, model, graph,args.use_dram)
